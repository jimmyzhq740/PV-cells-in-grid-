...
float open_loop, closed_loop=0.01; // Duty Cycles
...
void setup() {
  ...
  if(loopTrigger) { // This loop is triggered, it wont run unless there is an interrupt
    ...
    if (Boost_mode){
      if (CL_mode) { //Closed Loop Boost
        //starts MPPT algorithm via P&O
        if (current_mA==0){
          closed_loop=closed_loop;
          }
         else{
            dv=vb_10-v_initial;
            dp=p_in_mW-p_initial;
            if (abs(dp)<=1){         
              closed_loop=closed_loop;
              ...
              closed_loop=saturation(closed_loop,0.99,0.33);
 ...
 if (CL_mode){
      ...
      Serial.print("power: ");
      Serial.print(p_in_mW);
      Serial.print("\t\t");
      
      Serial.print("Inductor Current 2:  ");
...
void sampling(){
  ...
  if (Boost_mode == 1){
    ...
    vb_10=vb*10;
    p_in_mW=-i_dec_mA*vb;//in mili Watts
    dutyref = saturation(sensorValue2 * (1.0 / 1023.0),0.99,0.33);
  }else{
    iL = current_mA/1000.0;
    vb_10=vb*10;
    p_in_mW=-current_mA*vb;
